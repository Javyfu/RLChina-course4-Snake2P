    ...
    def run(self):
        ...
        for i_epoch in range(1, self.paras.max_episodes+1):
            ...
            while not self.g_core.is_terminal():
                ...
                next_state, reward, done, info_before, info_after, real_reward = self.env.step(joint_act)
            ...
            if self.paras.learn_terminal:
                self.agent.learn()
            print('i_epoch: ', i_epoch, 'Gt: ', '%.2f' % Gt,'Fruit_Gt: ', '%.2f' % Fruit_Gt, "epsilon: %.2f" % self.agent.agents[0].eps)
            reward_tag = 'reward'
