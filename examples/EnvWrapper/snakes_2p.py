...
class snakes_2p(BaseWrapper):
    ...
    def get_observationspace(self):
        ...
        self.obs_dim = 48 #2*self.num_agents + 14
    ...
    def step(self, action, train=True):
        ...
        step_reward = get_reward(info, ctrl_agent_index, reward) #reward #2021/12/17 fjw
        ...
        return next_state, step_reward, done, _, _,reward
...
def get_reward(info, snake_index, reward):
    ...
    for i in snake_index:
        if reward[i] > 0:
            ...
            step_reward[i] = 1/min(dists)*2

            dists = [np.sqrt(np.sum(np.square(other_snake - self_head))) for other_snake in snakes_position[1-i]]

            step_reward[i] -= 1 / min(dists)
